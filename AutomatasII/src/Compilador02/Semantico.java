...
public class Semantico {
	...
	ArrayList<Integer> abreLlaves;
	ArrayList<Integer> cierraLlaves;
	int desde, hasta;
	...
	public void localGlobal(Token token) { 
		String alcance="";
		abreLlaves = new ArrayList<Integer>();
		cierraLlaves = new ArrayList<Integer>();
		int renglon = 1;
		for(int i=0; i<Lexico.tokenAnalizados.size(); i++) { //Saber la posición del renglón donde estamos
			if(Lexico.tokenAnalizados.get(i).getValor().equals("{")) {
				abreLlaves.add(renglon);
			}
			if(Lexico.tokenAnalizados.get(i).getValor().equals("}")) {
				cierraLlaves.add(renglon);
			}
			if(Lexico.tokenAnalizados.get(i).getValor().equals("{") || Lexico.tokenAnalizados.get(i).getValor().equals("}") || Lexico.tokenAnalizados.get(i).getValor().equals(";")) {
				renglon++;
			}
		}
		
		for(int i=0; i<Lexico.tokenAnalizados.size(); i++) {
			if((Lexico.tokenAnalizados.get(i).getTipo() == "Identificador") && (Lexico.tokenAnalizados.get(i).getTipo() > abreLlaves.get(i)) && (Lexico.tokenAnalizados.get(i).getTipo() < cierraLlaves.get(i))) 
				alcance = "Local";
			else
				alcance = "Global";
		}
